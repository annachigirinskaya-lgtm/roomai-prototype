...
export async function POST(req:NextRequest){
  ...
  const isIncluded=plan!=='free'&&(isComparison||mode==='standard_design'||mode==='budget_remix'||mode==='product_swap');
  const unitCost=isComparison?CREDIT_COSTS.standard_design:(CREDIT_COSTS[mode]??CREDIT_COSTS.standard_design);
  const estimatedCost=isIncluded?0:unitCost*(isComparison?requested.length:1);
  ...
  const charged=isIncluded?0:unitCost*completed.length;
