...
export function billingReady(){
  return Boolean(process.env.ROOMAI_PUBLIC_BILLING_ENABLED==='true'&&process.env.NEXT_PUBLIC_SUPABASE_URL&&process.env.NEXT_PUBLIC_SUPABASE_PUBLISHABLE_KEY&&process.env.SUPABASE_SERVICE_ROLE_KEY&&process.env.STRIPE_SECRET_KEY&&process.env.STRIPE_WEBHOOK_SECRET&&process.env.STRIPE_PRICE_WEEKLY&&process.env.STRIPE_PRICE_MONTHLY&&process.env.STRIPE_PRICE_YEARLY&&process.env.NEXT_PUBLIC_APP_URL);
}
